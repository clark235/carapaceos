...
check('qemu-system-x86_64', () => {
  const path = which('qemu-system-x86_64');
  if (!path) return { ok: false, detail: 'Not found — install: apt install qemu-system-x86 / brew install qemu' };
  const ver = runVersion('qemu-system-x86_64');
  return { ok: true, detail: `${ver} at ${path}` };
});
